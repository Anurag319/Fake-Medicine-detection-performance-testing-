...
class OperationBase extends WorkloadModuleBase {
    ...
    async initializeWorkloadModule(workerIndex, totalWorkers, roundIndex, roundArguments, sutAdapter, sutContext) {
        ...
        this.assertSetting('manufacturername');
        this.assertSetting('username');
        this.assertSetting('mpid');
        this.assertSetting('upid');
        this.assertSetting('ppid');

        this.numberOfAccounts = this.roundArguments.numberOfAccounts;
        this.manufacturername = this.roundArguments.manufacturername;
        this.username = this.roundArguments.username;
        this.mpid = this.roundArguments.mpid;
        this.upid = this.roundArguments.upid;
        this.ppid = this.roundArguments.ppid;
        
        this.simpleState = this.createSimpleState();
        // console.log(`simplestate -- ${JSON.stringify(this.simpleState)}`);
    }
    ...
    _createEthereumConnectorRequest(operation, args) {
        // console.log(`operation2 - ${operation} -- args - ${JSON.stringify(args)}`)
        const query = operation === 'getProductHistory'
        return {
            contract: 'simple',
            verb: operation,
